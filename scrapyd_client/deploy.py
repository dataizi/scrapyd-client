...
def _build_egg(opts):
    ...
    _inject_requirements(egg)
    return egg, d


def _read_requirements():
    """Read the runtime requirements of the project in the current directory.

    utf-8-sig because a BOM would otherwise end up as part of the first
    requirement name and make it unparseable downstream.
    """
    if not os.path.isfile("requirements.txt"):
        _fail(
            "Error: Missing requirements.txt. Scrapyd verifies an egg's "
            "requirements against the packages installed on the server, so "
            "every egg must declare them."
        )
...
def _inject_requirements(egg_path):
    """Write requirements.txt into the egg as EGG-INFO/requires.txt.

    setuptools only emits that member when setup.py declares install_requires,
    which the autogenerated setup.py does not. Writing it here means every egg
    declares its requirements without each project having to maintain a
    setup.py.
    """
    requirements = _read_requirements()
    member = "EGG-INFO/requires.txt"
    # "\n" rather than os.linesep: egg metadata is read on the server, which is
    # not necessarily the platform the egg was built on.
    payload = "".join(line + "\n" for line in requirements)

    # A zip member cannot be replaced in place, so rebuild the archive.
    fd, tmp_path = tempfile.mkstemp(suffix=".egg", dir=os.path.dirname(egg_path))
    os.close(fd)
    with zipfile.ZipFile(egg_path, "r") as source:
    ...
    os.replace(tmp_path, egg_path)
    _log("Declared %d requirements in %s" % (len(requirements), member))
